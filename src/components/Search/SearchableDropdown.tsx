...
const SearchableDropdown: React.FC<SearchableDropdownProps> = ({
  ...
  const filterOptions = () => {
    return options.filter((option) => {
      ...
      return (
        optionLabel && optionLabel.toLowerCase().includes(query.toLowerCase())
      );
    });
  };
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <div
          className="dark:bg-customGray  bg-customWhiteGray absolute z-10 overflow-y-scroll h-36 pl-3 rounded-lg"
          style={{ transform: "translate(25%, 0px)" }}
        >
          {filterOptions().map((option, index) => (
            <div
              onClick={() => selectOption(option)}
              className={`dark:text-white text-black cursor-pointer hover:text-prime hover:dark:text-prime ${
